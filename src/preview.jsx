  ...
  render() {
    ...
    let items = this.state.data.map(item => {
      switch(item.element) {
        ...
        case 'Signature':
          return <Signature mutable={false} parent={this.props.parent} editModeOn={this.props.editModeOn} isDraggable={true} key={item.id} sortData={item.id} data={item} read_only={item.readOnly} defaultValue={this.state.answer_data[item.field_name]} _onDestroy={this._onDestroy} />;
        case 'HyperLink':
